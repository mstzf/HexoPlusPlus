...
function hpp_vi(id) {
    swal({ title: "\n修改中...", icon: "https://cdn.jsdelivr.net/gh/mstzf/HexoPlusPlus/stream/loading.gif", text: "\n", button: false, closeModal: false, });
    var ajax = ajaxObject();
...
function readFile() {
    swal({ title: "\n上传中...", icon: "https://cdn.jsdelivr.net/gh/mstzf/HexoPlusPlus/stream/loading.gif", text: "\n", button: false, closeModal: false, });
...
function hpp_edit(id, time, name, avatar, content, visible) {
    if(document.getElementById(id+'_edit_text')){
        return
    }
    var edit=document.createElement('div')
    edit.id=id+'_edit_text'
    edit.style.marginBottom='65px'
    edit.innerHTML = `
    ...
    <div class="black2">
        ...
        <button onclick="$('#input').click();" class="btn btn-primary"><i class="fa fa-photo fa-2x"><\/i><\/button>
        <button onclick="$('#upload_md').click();" class="btn btn-primary"><i class="fa fa-file fa-2x"><\/i><\/button>
        <button onclick="hpp_preview('hpp_talk_editor','talk')" id="hpp_eye_hpp_talk_editor" class="btn btn-primary"><i class="fa fa-eye fa-2x"><\/i><\/button>
        
    <\/div>   
    <textarea style="border:0;border-radius:5px;background-color:#90939920;width: 100%;min-height: 400px;padding: 10px;resize: none;display:block" id="${id}_talk_update_edi"><\/textarea><div style="border:0;border-radius:5px;background-color:#90939920;max-width: 100%;min-height: 70%;padding: 10px;resize: none;display: none;" id="${id}_div_talk_update_edi" class="hpp_pre_div"><\/div><div class="OwO" id="${id}_OwO"><\/div><button class="btn btn-primary pull-right" onclick="cancel(${id})">取消<\/button><button class="btn btn-primary pull-right" onclick="saveedit(${id})">保存更新<\/button><\/div>`

    setTimeout(function () {
        var OwO_edit = new OwO({
            container: document.getElementById(id+"_OwO"),
            api: hpp_OwO,
            position: 'down',
            maxHeight: '250px'
        });
        const talk_edit=document.getElementById(id+'_talk_update_edi')
        talk_edit.value=content
    }, 500);
    const talk = document.getElementById(id)

    insertAfter(edit, talk)

}
function saveedit(id){
    swal({ title: "\n上传中...", icon: "https://cdn.jsdelivr.net/gh/mstzf/HexoPlusPlus/stream/loading.gif", text: "\n", button: false, closeModal: false, });
    const div_talk_edit=document.getElementById(id+"_edit_text")
    const talk_edit=document.getElementById(id+'_talk_update_edi')
    let body = {
        "id":id,
        "content": talk_edit.value,
    }
    body = JSON.stringify(body)
    var ajax = ajaxObject();
    ajax.open("post", '/hpp/admin/api/updatetalk', true);
    ajax.setRequestHeader("Content-Type", "text/plain");
    ajax.onreadystatechange = function () {
        if (ajax.readyState == 4) {
            if (ajax.status == 200) {
